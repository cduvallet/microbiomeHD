...
It builds 10 classifiers based on a random subset of genera for
each dataset, where the number of genera selected is equal to the
number of core bugs which are present in that dataset.
"""
...
p.add_argument('--randomstate', help='random state seed for classifier',
    default=12345)
...
for dataset in dfdict.keys():
    ...
    for i in range(10):
        print(i)
        tmpdf = df[np.random.choice(df.columns, len(keep_bugs))]
        rf, X, Y = prep_classifier(tmpdf, H_smpls, dis_smpls, args.randomstate)
        ...
        resultsdf['num_features'] = tmpdf.shape[1]
        resultsdf['replicate'] = i
